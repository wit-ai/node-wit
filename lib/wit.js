...
function Wit(opts) {
  ...
  this.message = (message, context, n, verbose, junk) => {
    if (typeof message === 'object') {
      // User can pass an object with options, this will override arguments
      ({message, context, n, verbose, junk, msgId, threadId} = message)
    }
    ...
    if (threadId) {
      qs += '&thread_id=' + encodeURIComponent(threadId);
    }
    if (typeof n === 'number') {
