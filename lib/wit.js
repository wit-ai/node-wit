...
const makeWitResponseHandler = (endpoint, l, cb) => (
  response => {
    const err = (response instanceof Error) ? response : null ||
      response.data.error ||
      response.status !== 200 && response.data.body + ' (' + response.status + ')'
    ;
    if (err) {
    ...
    l.debug('[' + endpoint + '] Response: ' + JSON.stringify(response.data));
    if (cb) {
      process.nextTick(() => {
...
const Wit = function(token, actions, logger) {
  ...
  this.message = (message, cb) => {
    ...
    this.req.request(options)
      .then(makeWitResponseHandler('message', l, cb))
      .catch(makeWitResponseHandler('message', l, cb));
  };
  ...
  this.converse = (sessionId, message, context, cb) => {
    ...
    this.req.request(options)
      .then(makeWitResponseHandler('converse', l, cb))
      .catch(makeWitResponseHandler('converse', l, cb));
  };
