...
function Wit(opts) {
  ...
  const {
    accessToken, apiVersion, actions, headers, logger, witURL, maxSteps
  ...
  this.runActions = function(sessionId, message, context, maxSteps) {
    ...
    const steps = maxSteps || this.config.maxSteps || DEFAULT_MAX_STEPS;
...
const validate = (opts) => {
  ...
  if(!opts.maxSteps || !Number.isInteger(opts.maxSteps)) {
